...
\subsection{Hardware necessário}

\label{subsec:hardware}

A plataforma de voo (quadricóptero) que será utilizada nesse trabalho será a Parrot AR Drone 2. Esta é uma plataforma que vem sendo bastante utilizada no meio científico e seu modelo já está bem desenvolvido. Além disso, seu controle é realizado por WiFi e possui uma SDK (\textit{software development kit}) de código livre, o que facilita o desenvolvimento de algoritmos para seu controle.
...
Inicialmente, o trabalho será desenvolvido em um ambiente simulado MATLAB Simulink, para então ser testado no quadricóptero. Para simular a plataforma de voo, será utilizado o AR Drone Simulink Development-Kit para Matlab, que permite a simulação e o controle em tempo real do Parrot AR Drone 2. Já a obtenção de dados dos sensores e o desenvolvimento dos algoritmos serão realizados com o apoio do Raspberry Pi Support from MATLAB, que provê acesso aos periféricos através do Raspberry Pi, permitindo adquirir dados dos sensores conectados em tempo real.   

\subsection{Fluxo de trabalho}

Baseado nas etapas descritas na seção \ref{subsec:etapas} e os itens de hardware e software descritos nas seções \ref{subsec:hardware} e \ref{subsec:software}, o fluxo de trabalho proposto está ilustrado na Figura \ref{fig:Fluxo}. A fase de estudos envolve o aprofundamento sobre os aspectos teóricos descritos na seção \ref{subsec:etapas}. Em paralelo, o ambiente para simulação será montado enquanto as aquisições necessárias serão realizadas. Após, a implementação da solução será realizada de forma incremental, com o primeiro ciclo utilizando apenas um sensor ultrassônico para validação dos algoritmos de detecção de obstáculo e identificação de colisão. Em seguida, serão adicionado outros sensores na solução em um segundo ciclo. Por fim, a plataforma então será utilizada, para realização de testes em campo e o embarque da solução completa.  
...
O advento da utilização dos VANT's, mais especificamente os quadricópteros, em ambientes restritos, trazem a tona a necessidade de controlar o desvio de obstáculo de forma automática. Este é um tema que tem merecido atenção na área de robótica e nossa proposta visa explorá-lo com medições de sensores em tempo real, auxiliando o controle do veículo. Com esse enfoque, acreditamos que esta é uma proposta válida e que terá ampla aplicação na área de defesa. Além disso, esta solução será construída para ser futuramente utilizada como o módulo de desvio de obstáculos da plataforma VANT-IME, que está em fase de concepção no Laboratório de Robótica do IME.
