...
\section{O Problema: Segurança em voo para quadricoptero}
\label{sec:prob}
...
\begin{itemize}
	\item
	Estudar o SDK de controle da plataforma Parrot Ar Drone 2.  
\end{itemize}

\noindent\textbf{Sensores IMU} 
\begin{itemize}
	\item
	Verificar como obter os dados da IMU da plataforma Parrot Ar Drone 2 através de sua SDK.
\end{itemize}

\noindent\textbf{Fusão e Filtragem da IMU} 
\begin{itemize}
	\item
	Estudar a utilização dos dados já filtrados pela plataforma Parrot Ar Drone 2.  
\end{itemize}
...
\begin{itemize}
	...
	\item
	Estudar a dinâmica de voo da plataforma Parrot Ar Drone 2 e implementar a extrapolação de sua posição em um dado período de tempo.  
\end{itemize}

\noindent\textbf{Controle de Atitude} 
\begin{itemize}
	\item
	Verificar o padrão de troca de comandos entre os comandos de controle e a controladora de voo para implementação através da SDK.  
\end{itemize}
...
A viabilidade desta proposta depende da disponibilização dos itens necessários para implementação e testes do método. A partir dos componentes e software necessários descritos nas seções \ref{subsec:hardware} e \ref{subsec:software}, foi feito um levantamento dos itens já disponíveis em laboratório para teste e os que necessitam ser adquiridos. A seguir está a listagem dos itens envolvidos e seu status de disponibilidade.

\begin{itemize}
	\item
	Plataforma de voo: O laboratório de Robótica do IME dispõe de quatro quadricópteros para teste, com aproximadamente 45cm de diâmetro, com capacidade de carga suficiente para embarcar um mini computador e sensores ultrassônicos.
	\item
	Minicomputador: Estão disponíveis dois minicomputadores Raspberry PI modelo B com processador de 700MHz, 512MB de memória, duas saídas USB e portas Ethernet para rede para testes no mesmo laboratório.
	\item
	Sensores ultrassônicos: O laboratório de Robótica do IME dispõe de sensores ultrassônicos modelo XXXXX que serão testados inicialmente e terão sua capacidade avaliada. Caso haja a necessidade de aquisição, são itens de baixo custo e facilmente encontrados no mercado brasileiro.
	\item
	MatLab Simulink: Será usado o Matlab R2013a com Simulink, versão para estudante que possui todas as ferramentas necessárias para simulação do problema. 

\end{itemize}

Conforme a listagem acima, estão disponíveis os itens necessários para o começo dos trabalhos e realização dos primeiros testes, o que atesta a viabilidade desta proposta.
...
O advento da utilização dos VANT's, mais especificamente os quadricópteros, em ambientes restritos, trazem a tona a necessidade de controlar o desvio de obstáculo de forma automática. Este é um tema que tem merecido atenção na área de robótica e nossa proposta visa explorá-lo com medições de sensores em tempo real, auxiliando o controle do veículo. Com esse enfoque, acreditamos que esta é uma proposta válida e que terá ampla aplicação na área de defesa. Além disso, será construída uma solução que futuramente será utilizada como o módulo de desvio de obstáculos da plataforma VANT-IME, que está em fase de concepção no Laboratório de Robótica do IME.
