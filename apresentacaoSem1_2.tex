...
\begin{frame}[allowframebreaks]
	...
	\begin{itemize}
		...
		\item Seis graus de liberdade: Três são variações lineares ($x$,$y$,$z$) e três são angulares ($\phi$,$\theta$,$\psi$)
						
		\item  Sistema sub-atuado (quatro atuadores), dimensão do espaço de configurações ($x$,$y$,$z$,$\phi$,$\theta$,$\psi$) excede o espaço do controle ($u_1$,$u_2$,$u_3$,$u_4$) 
...
%		
	\framebreak
		...
		\begin{figure}
			...
			\caption{Estrutura e movimentos da plataforma. Fonte \cite{Mian2008}}
...
\begin{frame}
	...
	\item Fornecidas por sensores inerciais: acelerômetros e girômetros
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item Três acelerômetros: aceleração linear ($\ddot{x}$,$\ddot{y}$,$\ddot{z}$)
		
		\item Três girômetros: variação angular em seu próprio eixo  ($\dot{\phi}$,$\dot{\theta}$,$\dot{\psi}$) \cite{crivella15}
		...
		\begin{figure}[h]
			...
			\caption{Estrutura do Sistema de Navegação acoplada ao veículo (esquerda) e os movimentos associados no quadricóptero (direita). Fonte \cite{Adalberto2009}}
			\label{fig:imuStrap}
		\end{figure}
	\end{itemize}
\end{frame}
...
%		\end{figure}
		...
		\item Simulação e validação \textit{hardware-in-the-loop}
...
\begin{frame}
	...
	\frametitle{Andamento do Trabalho}
